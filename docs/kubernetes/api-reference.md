...
| `runtimeVersion` _string_ | RuntimeVersion is the Dynamo runtime compatibility version for this component.<br />When omitted, the defaulting webhook derives it from a parseable main<br />container image tag such as `vllm-runtime:1.1.0` -> `1.1`. Set this<br />explicitly when using SHA-tagged or custom runtime images. |  | Optional: \{\} <br /> |
...
| `runtimeVersion` _string_ | runtimeVersion is the Dynamo runtime compatibility version for this<br />component. When omitted, the defaulting webhook derives it from a<br />parseable main container image tag such as `vllm-runtime:1.1.0` -> `1.1`.<br />Set this explicitly when using SHA-tagged or custom runtime images. |  | Optional: \{\} <br /> |
